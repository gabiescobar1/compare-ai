...
export default function ModelSelector({ selectedModels, onChange, disabled }) {
  ...
  const renderPair = (selection, index) => {
    ...
    return (
      <div
        key={index}
        className="flex items-center gap-2 bg-slate-50 dark:bg-[#1e1410] rounded-2xl p-3 border border-slate-200 dark:border-white/8 transition-all hover:border-slate-300 dark:hover:border-white/15"
      >
        {/* Provider dropdown */}
        <div className="relative flex-1 min-w-0">
          <select
            disabled={disabled}
            className="w-full bg-white dark:bg-[#2a1c14] border border-slate-300 dark:border-white/10 text-slate-800 dark:text-[#f0e4d4] text-sm font-bold rounded-xl shadow-sm px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#ff6b00] appearance-none disabled:opacity-50 transition-colors truncate"
        ...
        <div className="relative flex-1 min-w-0">
          <select
            disabled={disabled}
            className="w-full bg-white dark:bg-[#2a1c14] border border-slate-300 dark:border-white/10 text-slate-800 dark:text-[#f0e4d4] text-sm font-medium rounded-xl shadow-sm px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#ff6b00] appearance-none disabled:opacity-50 transition-colors truncate"
  ...
  return (
    <div className="mb-8 border-b border-slate-200 dark:border-white/8 pb-8">
      <label className="block text-slate-700 dark:text-[#c4b09a] text-sm font-bold mb-4">
        Modelos de IA
      </label>
        ...
        <button
          ...
          className={`flex-shrink-0 p-3 rounded-2xl border-2 transition-all ${
            canAdd && !disabled
              ...
              : 'bg-slate-100 dark:bg-[#2a1c14] text-slate-400 dark:text-[#6a5040] border-slate-200 dark:border-white/8 cursor-not-allowed'
          }`}
        >
